...
from utils.aliyun.com.aliyun.api.gateway.sdk.auth import sha_hmac256
...
def _build_resource(uri="", body={}):
    ...
    if body:
        ...
        for key in param_list:
            # resource.append(urllib.urlencode({key:body[key].encode("utf-8")}))
            if not first:
                resource.append("&")
            first = False
            ...
            if body[key]:
                resource.append(key)
                resource.append("=")
                resource.append(urllib.quote(body[key].encode("utf-8")))
            else:
                resource.append(key)
    s = ''.join(resource)
