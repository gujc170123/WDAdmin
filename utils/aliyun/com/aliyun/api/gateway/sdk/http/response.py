    ...
    def get_http_response(self):
        ...
        try:
            ...
            if self.get_content_type() == constant.CONTENT_TYPE_FORM and self.get_body():
                ...
                post_data.append("Signature=%s" %signature)
                bks = body.keys()
                bks.sort()
                for bk in bks:
                    # post_data.append(urllib.urlencode({bk: body[bk]}))
                    s = "%s=%s" %(bk, body[bk].decode('utf-8'))
                    post_data.append(s.encode("utf-8"))
                post_data = "&".join(post_data)

            else:
                post_data = self.get_body()
            response = urllib.urlopen(self.get_url(), post_data)
            return None, None, response.read()
