...
def enterprise_account(auth_obj):
    ...
    # except:
    #     info_logger.info("user_id %s has error authuser" % user_id)   # 没有这个Authuser
    #     return None
    user_id = auth_obj.id
    if not auth_obj.account_name:
        info_logger.info("user_id %s has no account" % user_id)   # 用户没有账户名
        return None
    try:
        peo_obj = People.objects.get(user_id=user_id)
    except:
        info_logger.info("user_id %s has error People" % user_id)  # 用户没有 people
        return None
    # 可以参与多个项目
    if peo_obj:
        assuser_qs = AssessUser.objects.filter_active(people_id=peo_obj.id).values_list("assess_id", flat=True)
        if not assuser_qs:
            info_logger.info("user_id %s has error assess" % user_id)
            # 用户没有项目
            return None
        ...
        for ass_enter in ass_enter_ids:
            ...
            if old_ea_qs_all.count() > 1:
                err_logger.error('ERROR ea_id %s account %s double error' % (old_ea_qs_all[0].id, old_ea_qs_all[0].account_name))
                # 同一企业有2个账户
                return None
            elif old_ea_qs_all.count() == 1:
                if old_ea_qs_all[0].user_id != user_id:
                    debug_logger.debug('user_id %s NEED DEBUG ea_id %s account %s has user_id %s error with new user_id' % (user_id, old_ea_qs_all[0].id, old_ea_qs_all[0].account_name, old_ea_qs_all[0].user_id))
                    # 企业账户名已被使用
                    return None
                else:
                    info_logger.info("user_id %s HAS EXISTS ea_id %s account %s has user_id" % (old_ea_qs_all[0].user_id, old_ea_qs_all[0].id, old_ea_qs_all[0].account_name))
                    # 你已经存在了
                ...
                info_logger.info("user_id %s, success ea_id %s new " % (user_id, ea.id))
                # 成功
