...
from assessment.models import AssessOrganization
...
logger = get_logger("write_hive_utils")
...
def etl_write_people_info_file(file_name, contents, t, timestamp):
    file_path = os.path.join("download", "etl", t, timestamp)
    if not os.path.exists(file_path):
        os.makedirs(file_path)
    file_full_path = os.path.join(file_path, file_name)
...
def etl_write_sign_info_file(file_name, content, t, timestamp):
    file_path = os.path.join("download", "etl", t, timestamp)
    if not os.path.exists(file_path):
        os.makedirs(file_path)
    file_full_path = os.path.join(file_path, file_name)
